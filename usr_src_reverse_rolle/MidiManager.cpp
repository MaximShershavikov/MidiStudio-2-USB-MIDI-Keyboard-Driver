    ...
    while (true)
    {
        ...
        if (hFile != INVALID_HANDLE_VALUE)
        {
            ...
            if (NumberOfBytesRead < MAX_BUF_SIZE)
            {
                ...
                if (MidiInfo->PtrBufferWord[0] == PITCH_BEND_ROLLE_HEAD_1 && MidiInfo->PtrBufferWord[2] == PITCH_BEND_ROLLE_HEAD_2)
                {
                    MidiInfo->NewMessage[3] = UP_ROLLE_POS - MidiInfo->NewMessage[3];
                    MidiInfo->NewMessage[7] = UP_ROLLE_POS - MidiInfo->NewMessage[7];
                }
                else if (MidiInfo->PtrBufferWord[0] == MOD_ROLLE_HEAD_1 && MidiInfo->PtrBufferWord[2] == MOD_ROLLE_HEAD_2)
                {
                    MidiInfo->NewMessage[3] = UP_ROLLE_POS - MidiInfo->NewMessage[3];
                    MidiInfo->NewMessage[7] = UP_ROLLE_POS - MidiInfo->NewMessage[7];
                }
